...
for maxevals = [10]
  fid = fopen(['direct-usresults-1-10-' num2str(probinds(1)) ...
  ...
  while (todo > 0)
    ...
    for pind=1:numel(inproblems)
      ...
      cd ourresults
      if (exist(['direct-out-1-10-' inproblem.name '-' num2str(maxevals) '.mat']) == 2)
        eval(['load direct-out-1-10-' inproblem.name '-' num2str(maxevals)]);
      else
        outproblems = [];
        off = 1;
      end
      ...
      problem.ourinsamples = [5];
      problem.ouroutsamples = [9];
      ...
      cd ourresults
      eval(['save -v7.3 direct-out-1-10-' problem.name '-' num2str(maxevals) ' outproblems off']);
      cd '..'
		end
