...
for maxevals = [10]
  fid = fopen(['direct2-redo-usresults-1-10-' num2str(probinds(1)) ...
  ...
  while (todo > 0)
    ...
    for pind=1:numel(inproblems)
      ...
      cd ourresults
      if (exist(['direct2-out-1-10-' inproblem.name '-' num2str(maxevals) '.mat']) == 2)
        eval(['load direct2-out-1-10-' inproblem.name '-' num2str(maxevals)]);
      else
        outproblems = [];
        off = 1;
      end
      ...
      disp(['offset number: ' num2str(off)]);
      ...
      for sind = 1:numel(problem.ourinsamples)
        for covvyind = 1:length(covvys)
          ...
          options = struct( ...
              'function_evaluations', maxevals * problem.n, ...
              'lookahead_steps', 2, ...
              'derivative_observations', derivobs ...
              );
          ...
          tic
					o = minimise(problem.f,problem.x0,problem.lowrbnd, ...
											 problem.upprbnd,problem.covvy,options);
...
%           disp(['our mxerror: ' num2str(problem.ourmxerrors(end))]);
      ...
      cd ourresults
      eval(['save direct2-out-1-10-' problem.name '-' num2str(maxevals) ...
						' outproblems off']);
      cd '..'
		end
