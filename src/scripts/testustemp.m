...
for maxevals = [10]
  fid = fopen(['testing-usresults-1-10-' num2str(probinds(1)) ...
  ...
  while (todo > 0)
    ...
    for pind=1:numel(inproblems)
      ...
      cd ourresults
      if (exist(['testing-out-1-10-' inproblem.name '-' num2str(maxevals) '.mat']) == 2)
        eval(['load five-out-1-10-' inproblem.name '-' num2str(maxevals)]);
      else
        outproblems = [];
        off = 1;
      end
	...
	problem.f = @(x)-problem.forig(x);
      ...
      problem.ourinsamples = [1 7];
      problem.ouroutsamples = [1 15];
			if (problem.periodic)
      ...
      for sind = 1:numel(problem.ourinsamples)
        for covvyind = 1:length(covvys)
          derivobs = (~problem.periodic && covvyind == 2);
          ...
          problem.covvy
	if (derivobs)
	  problem.covvy.meanfn=@(hp) ndimssqdexp_mean_fn(hp);
	end
	...
	if (derivobs)
		options.MaxFunEvals = 1;
          ...
          if (~problem.periodic || covvyind == 1)
            ...
            problem.covvy.hyperparams(4) = ...
              struct('name','logNoiseSD', ...
						...
						options.MaxFunEvals = 1; % crazy line!!
            problem.covvy.hyperparams(1) = ...
              struct('name','logInputScale', ...
										 'priorMean',log(problem.inscale), ...
										 'priorSD',problem.instd, ...
      ...
      %cd ourresults
      %eval(['save -v7.3 five-out-1-10-' problem.name '-' num2str(maxevals) ' outproblems off']);
      %cd '..'
		end
